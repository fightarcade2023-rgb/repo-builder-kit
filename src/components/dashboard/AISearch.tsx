  ...
  const searchInternalData = async (query: string) => {
    ...
    if (matchingProducts.length > 0) {
      ...
      matchingProducts.slice(0, 3).forEach((p: any) => {
        results += `• ${p.product_name} - R$ ${p.sale_price} (Estoque: ${p.quantity})\n`;
      });
      results += "\n";
    }
    ...
    if (matchingSales.length > 0) {
      ...
      matchingSales.slice(0, 3).forEach((s: any) => {
        results += `• ${s.product_name} - ${s.customer_name} - R$ ${s.total_revenue?.toFixed(2)}\n`;
      });
      results += "\n";
    }
    ...
    if (matchingServices.length > 0) {
      ...
      matchingServices.slice(0, 3).forEach((s: any) => {
        results += `• ${s.service_name} - ${s.client_name} - R$ ${s.total_value?.toFixed(2)}\n`;
      });
      results += "\n";
    }

    if (!matchingProducts.length && !matchingSales.length && !matchingServices.length) {
      ...
      <Card className="shadow-lg border-0 bg-gradient-to-r from-slate-900 to-blue-900 text-white">
        <CardContent className="p-6">
          ...
          <form onSubmit={handleSearch} className="space-y-3">
            ...
            {searchResults && (
              <div className="mt-4 p-4 bg-white/10 rounded-lg text-white whitespace-pre-wrap text-sm">
                {searchResults}
              </div>
            )}
          </form>
